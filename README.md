...
- OpenAI-compatible 模型调用层，支持 DeepSeek、Qwen/DashScope、OpenAI、Moonshot、Zhipu、SiliconFlow、OpenRouter、Volcengine Ark 和自定义兼容接口
...
- Evidence PDRS / Roadmap / PRD Review Gate：在 PRD 之后生成证据化评分、阶段化路线图和六角色 PRD 审查结果，并写入结构化 artifact 与 Markdown artifact，作为 Codex Pack 导出前的质量控制输入。
...
模型配置可在 Hermes 控制面板保存。保存内容位于 `.next/hermes/model-config.json`，优先级高于环境变量；如果未保存，则回退到 `HERMES_LOCAL_PROVIDER`、`HERMES_LOCAL_MODEL`、`HERMES_INFERENCE_PROVIDER` 和 `HERMES_INFERENCE_MODEL`。保存模型配置只写入配置文件，不会隐式扫描或启动 Hermes dashboard；运行状态由控制面板的状态接口单独读取。未显式填写模型名时，系统会按 provider 使用默认模型，例如 `deepseek-chat`、`qwen-plus`、`gpt-4.1-mini` 或对应供应商的默认值。

当前模型调用只支持 `api` 模式。历史保存的 `codex-cli` provider 或 usageMode 会被规范化为 API provider 配置，不作为本地 Hermes 运行方式。
...
- `HERMES_AGENT_LOG_DELTA_MAX_BYTES`：local Hermes 调用后最多读取的 agent log 增量字节数，用于解析 Tool 使用情况；默认 524288 字节，避免大日志造成内存尖峰。
...
- `MOONSHOT_API_KEY`、`MOONSHOT_BASE_URL`：Moonshot/Kimi 兼容接口的密钥和可选自定义地址。
- `ZHIPU_API_KEY`、`ZHIPU_BASE_URL`：智谱 GLM 兼容接口的密钥和可选自定义地址。
- `SILICONFLOW_API_KEY`、`SILICONFLOW_BASE_URL`：SiliconFlow 兼容接口的密钥和可选自定义地址。
- `OPENROUTER_API_KEY`、`OPENROUTER_BASE_URL`：OpenRouter 兼容接口的密钥和可选自定义地址。
- `VOLCENGINE_API_KEY`、`VOLCENGINE_BASE_URL`：火山方舟兼容接口的密钥和可选自定义地址。
- `CUSTOM_OPENAI_API_KEY`、`CUSTOM_OPENAI_BASE_URL`、`OPENAI_COMPATIBLE_API_KEY`、`OPENAI_COMPATIBLE_BASE_URL`：自定义 OpenAI-compatible 接口的密钥和地址。
